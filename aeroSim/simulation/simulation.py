...
class Simulation:
    ...
    def run_test(self, map_name: str = "default", particle_count: int = 1000) -> dict:
        ...
        while self.engine.is_running and self.world.particles_exited < self.world.target_particles:
            ...
            # Inicia cronômetro quando primeira partícula sai
            if self.world.particles_exited > 0 and not started_timing:
                started_timing = True
                timing_start = time.time()
                last_particle_exit_time = timing_start
            
            # Atualiza tempo de última queda
            if self.world.particles_exited > 0:
                last_particle_exit_time = time.time()
            ...
            if started_timing and last_particle_exit_time:
        ...
        print(f"Resultado: Tempo = {total_time:.2f}s | Fluxo = {particles_per_second:.2f} partículas/s")
